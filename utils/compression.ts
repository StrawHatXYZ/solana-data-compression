...
export async function createTree(
  ...
  const createTreeIx = createInitEmptyMerkleTreeIx(
    treeKeypair.publicKey, 
    payer.publicKey, 
    maxDepthSizePair
);
  ...
  try {
    ...
    console.log("\nMerkle tree created successfully!",payer.publicKey,treeKeypair.publicKey);
...
interface UserData {
  productId: string; // Hash of Amazon URL
  productName: string;
  productDescription: string;
  productLink: string;
  discoverers: string[]; // Array of discoverer names
}

// import { Transaction, sendAndConfirmTransaction} from "@solana/web3.js";
import { createAppendIx ,createInitEmptyMerkleTreeIx} from "@solana/spl-account-compression";
import crypto from "crypto";

export async function appendUserDataToTree(
  connection: Connection,
  payer: Keypair,
  ...
  // Serialize the user data into a Buffer
  const serializedData = Buffer.from(JSON.stringify(userData));

  // Create a new leaf, which is a hash of the serialized data
  const newLeaf = crypto.createHash('sha256').update(serializedData).digest();

  // Create the append leaf instruction
  // const appendIx = createAppendIx(payer.publicKey,treeKeypair.publicKey, newLeaf);
  const appendIx = createAppendIx(treeAddress,payer.publicKey, newLeaf);

  // Create a transaction and add the append leaf instruction
  const tx = new Transaction().add(appendIx);
  tx.feePayer = payer.publicKey;

  // Send and confirm the transaction
  const txSignature =await sendAndConfirmTransaction(connection, tx, [payer]);
  console.log("\nData added to the tree!");
  console.log(explorerURL({ txSignature }));
